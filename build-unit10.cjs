...
const css=fs.readFileSync('assets/day10.css','utf8');
const code=['day10.js','day10-runtime.js'].map(f=>fs.readFileSync('assets/'+f,'utf8')).join('\n');
const html=`<!doctype html>
<html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Travel Lab｜第 10 天：請求東西、婉拒與道歉</title>
<meta name="description" content="成人旅遊英文第 10 天：請求物品、婉拒提供與邀請、道歉與補救。七部分互動教材。">
...
${sharedStyles}<style id="day10-styles">${css}</style></head><body>
...
<nav class="day-switcher" aria-label="切換單元"><div class="day-switcher-inner"><span class="day-switcher-label">切換單元</span><div class="day-switcher-links">${pages.map(([file,n,name])=>`<a href="${file}"${n===10?' aria-current="page"':''}><strong>第 ${n} 天</strong><small>${name}</small></a>`).join('')}</div></div></nav>
<div id="storageNotice" class="notice" role="status" hidden></div>
<div class="layout"><details id="courseMenu" class="course-menu" open>${menuSummary}<nav class="course-nav" aria-label="第 10 天的七部分課程"><span class="day">DAY 10</span><div id="nav"></div></nav></details><main id="main" tabindex="-1"></main></div>
<footer class="bottom"><a href="index.html">全部課程</a> · 第 10 天：請求東西、婉拒與道歉</footer><div id="toast" role="status" hidden></div>
<script id="day10-app">${code}</script></body></html>`;
fs.writeFileSync('travel-english-unit10-kind-words.html',html);
require('./sync-unit10-links.cjs')();
console.log('Built Day 10: standalone HTML with all seven sections and embedded assets.');
...
require('./sync-speaking-focus.cjs')('travel-english-unit10-kind-words.html');
