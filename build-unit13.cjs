...
const css=fs.readFileSync('assets/day13.css','utf8');
const code=['day13.js','day13-runtime.js'].map(f=>fs.readFileSync('assets/'+f,'utf8')).join('\n');
const html=`<!doctype html>
<html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Travel Lab｜第 13 天：一起決定去哪裡：Why／Which</title>
<meta name="description" content="成人旅遊英文第 13 天：和旅伴選景點、說理由、提建議與回應。七部分互動教材。">
...
${sharedStyles}<style id="day13-styles">${css}</style></head><body>
...
<nav class="day-switcher" aria-label="切換單元"><div class="day-switcher-inner"><span class="day-switcher-label">切換單元</span><div class="day-switcher-links">${pages.map(([file,n,name])=>`<a href="${file}"${n===13?' aria-current="page"':''}><strong>第 ${n} 天</strong><small>${name}</small></a>`).join('')}</div></div></nav>
<div id="storageNotice" class="notice" role="status" hidden></div>
<div class="layout"><details id="courseMenu" class="course-menu" open>${menuSummary}<nav class="course-nav" aria-label="第 13 天的七部分課程"><span class="day">DAY 13</span><div id="nav"></div></nav></details><main id="main" tabindex="-1"></main></div>
<footer class="bottom"><a href="index.html">全部課程</a> · 第 13 天：一起決定去哪裡：Why／Which</footer><div id="toast" role="status" hidden></div>
<script id="day13-app">${code}</script></body></html>`;
fs.writeFileSync('travel-english-unit13-why-which.html',html);
require('./sync-unit13-links.cjs')();
console.log('Built Day 13: standalone HTML with all seven sections and embedded assets.');
...
require('./sync-speaking-focus.cjs')('travel-english-unit13-why-which.html');
